...
Since I want to do a t-test between ambient and treatment samples, I should combine samples by exposure.

```{r}
ambientSamples <- rbind(zr2096_1,
                        zr2096_2,
                        zr2096_3,
                        zr2096_4,
                        zr2096_5) #Combine all ambient samples
treatmentSamples <- rbind(zr2096_6,
                          zr2096_7,
                          zr2096_8,
                          zr2096_9,
                          zr2096_10) #Combine all treatment samples
```

```{r}
#Remove extraneous columns
ambientSamples <- ambientSamples[,-c(1, 6:8)]
treatmentSamples <- treatmentSamples[,-c(1, 6:8)]
head(treatmentSamples) #Confirm column deletion
```

```{r}
#Calculate median percent methylation by gene
ambientPercentMeth <- aggregate(percentMeth ~ geneID, data = ambientSamples, FUN = median)
treatmentPercentMeth <- aggregate(percentMeth ~ geneID, data = treatmentSamples, FUN = median)
head(ambientPercentMeth) #Confirm percent methylation calculation
```
