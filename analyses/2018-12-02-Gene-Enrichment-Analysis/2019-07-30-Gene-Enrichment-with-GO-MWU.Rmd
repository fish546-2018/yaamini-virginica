...
GO-MWU takes all loci into account, but there may be some differences in parental GOslim terms if I only look at differentially methylated loci. I will pull loci that have at least a 50% difference between treatment, p-value < 0.05, and q-value < 0.01 from my condensed list (only one DML per gene) so it is comparable with the list I generated for all tested loci above.
...
```{r}
parentGOBPCondensed <- merge(x = condensedDML, y = parentGOBP, by = "Genbank") #Merge condensed DML with parentGOBP
head(parentGOBPCondensed) #Confirm merge
```

```{r}
parentGOBPCondensedCounts <- as.data.frame(table(parentGOBPCondensed$name)) #Count the frequency of each GOslim term
colnames(parentGOBPCondensedCounts) <- c("parentGOslim", "frequency") #Rename columns
parentGOBPCondensedCounts <- subset(parentGOBPCondensedCounts, subset = frequency > 0) #Remove residual factors
head(parentGOBPCondensedCounts) #Confirm counts
```

```{r}
write.csv(parentGOBPCondensedCounts, "2019-07-30-condensedDML-Parent-GOSlim-Frequency-BP.csv", row.names = FALSE, quote = FALSE) #Save file
```
...
```{r}
condensedDMLHyper <- subset(condensedDML, subset = meth.diff > 0) #Subset hypermethylated loci
range(condensedDMLHyper$meth.diff) #Confirm subset
```

```{r}
parentGOBPCondensedHyper <- merge(x = condensedDMLHyper, y = parentGOBP, by = "Genbank") #Merge condensed DML with parentGOBP
...
```{r}
condensedDMLHypo <- subset(condensedDML, subset = meth.diff < 0) #Subset hypermethylated loci
range(condensedDMLHypo$meth.diff) #Confirm subset
```

```{r}
parentGOBPCondensedHypo <- merge(x = condensedDMLHypo, y = parentGOBP, by = "Genbank") #Merge condensed DML with parentGOBP
...
```{r}
parentGOCCCondensed <- merge(x = condensedDML, y = parentGOCC, by = "Genbank") #Merge condensed DML with parentGOCC
...
```{r}
parentGOCCCondensedHyper <- merge(x = condensedDMLHyper, y = parentGOCC, by = "Genbank") #Merge condensed DML with parentGOCC
...
```{r}
parentGOCCCondensedHypo <- merge(x = condensedDMLHypo, y = parentGOCC, by = "Genbank") #Merge condensed DML with parentGOCC
...
```{r}
parentGOMFCondensed <- merge(x = condensedDML, y = parentGOMF, by = "Genbank") #Merge condensed DML with parentGOMF
...
```{r}
parentGOMFCondensedHyper <- merge(x = condensedDMLHyper, y = parentGOMF, by = "Genbank") #Merge condensed DML with parentGOMF
...
```{r}
parentGOMFCondensedHypo <- merge(x = condensedDMLHypo, y = parentGOMF, by = "Genbank") #Merge condensed DML with parentGOMF
...
gomwuStats(input, goDatabase, goAnnotations, goDivision,
	...
	smallest=5,   # a GO category should contain at least this many genes to be considered
	clusterCutHeight=0.25) # threshold for merging similar (gene-sharing) terms. See README for details.
# There are no GO terms pass 10% FDR. The output is also identical to the previous GO-MWU output with signed log p-values, so I won't proceed down this route with other GO divisions.
```
