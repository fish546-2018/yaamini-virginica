...
```{r}
parentGOBPDML <- unique(merge(x = DMLGenbank, y = parentGOBP, by = "Genbank")) #Merge parent GOslim terms and DML information by Genbank ID. Keep only unique entries. This will allow me to retain multiple DML per gene, but remove any merging artifacts.
head(parentGOBPDML) #Confirm merge. There are 59 entries, so much of the DML may be in uncharacterized transcripts.
```
...
```{r}
parentGOBPCondensedHyper <- unique(merge(x = condensedDMLHyper, y = parentGOBP, by = "Genbank")) #Merge condensed DML with parentGOBP
...
```{r}
parentGOBPCondensedHypo <- unique(merge(x = condensedDMLHypo, y = parentGOBP, by = "Genbank")) #Merge condensed DML with parentGOBP
...
```{r}
parentGOCCCondensed <- unique(merge(x = DMLGenbank, y = parentGOCC, by = "Genbank")) #Merge condensed DML with parentGOCC
...
```{r}
parentGOMFCondensed <- merge(x = DMLGenbank, y = parentGOMF, by = "Genbank") #Merge condensed DML with parentGOMF
