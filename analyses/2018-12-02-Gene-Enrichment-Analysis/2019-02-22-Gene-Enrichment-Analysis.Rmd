...
```{r}
combDMLBreakdown <- rbind(genes2DMLBreakdown,
                          genes3DMLBreakdown,
                          genes4DMLBreakdown)
...
combDMLCounts[,5] <- c(0, 0, 0, 0, 1) #Add data for one gene with 5 DML (4 hyper, 1 hypo)
...
colnames(combDMLCounts) <- c(2, 3, 4, 5) #Add total number of DML per gene as column names
...
plotMatrix <- matrix(c(1,1,0,3,
                       2,2,0,3),
                     nrow = 2, ncol = 4, byrow = TRUE) #Create a matrix with plot location information. Fill matrix by row.
par(mar = c(0, 0, 6, 2.5), oma = c(6, 8, 0, 0)) #Specify inner and outer margins
layout(mat = plotMatrix, width = c(2, 2, 1.4, 2)) #Create a layout based on plotMatrix. The first and second plots (first column) should be wider.
...
DMLbarplot <- barplot(as.matrix(t(DMLchrCounts$DMLbyChrCpG)),
                      ...
                      xlim = c(0.7,11.5),
                      ylim = c(0,7e-05),
                      col = plotColors[7]) #Create a barplot and save as a new object. Use axes = FALSE to remove the y-axis and names.arg to set labels on the x-axis. The object contains x coordinates for bars, so xlim is set at 12 to compensate for maximum value of 11.5
...
mtext(side = 2, "Number DML per 10,000 CpGs", line = 5, cex = 1.3) #Add y-axis label for DML counts
...
axis(side = 2, at = seq(0, 100, by = 10), las = 2, col = "grey80", cex.axis = 1.2) #Add y-axis
mtext(side = 2, "Hypermethylated DML (%)", line = 4, cex = 1.3) #Add y-axis label
mtext(side = 3, line = -1, adj = 0, text = "  c.") #Add figure label

legend(x = 0.5, y = 105.1,
       legend = c("0", "1", "2", "3", "4"),
       horiz = TRUE,
       pch = 22, 
        col = "black", 
