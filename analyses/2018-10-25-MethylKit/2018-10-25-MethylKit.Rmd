...
par(mar = c(4, 4, 1, 1)) #Specify inner and outer margins
...
points(fig.allDataPCA, "sites", col = c(rep(plotColors[2], times = 5), rep(plotColors[4], times = 5)), pch = 16, cex = 2) #Add each sample. Darker samples are ambient, lighter samples are elevated pCO2
...
axis(side =  1, labels = TRUE, col = "grey80", cex.axis = 0.75) #Add x-axis
mtext(side = 1, text = "PC 1 (18.1%)", line = 2.5) #Add x-axis label

axis(side =  2, labels = TRUE, col = "grey80", cex.axis = 0.75) #Add y-axis
mtext(side = 2, text = "PC 2 (11.7%)", line = 2.5) #Add y-axis label

legend("topright", 
       pch = 16, 
       legend = c("Ambient", "Elevated"), 
       col = c(plotColors[2], plotColors[4]), 
       cex = 1, bty = "n") #Add a legend with information about ambient and elevated samples
...
```{r}
DMLMatrix <- methylationInformationFilteredCov5Destrand[differentialMethylationStatsFilteredCov5Destrand$start == methylationInformationFilteredCov5Destrand$start,]
head(DMLMatrix)
length(DMLMatrix$chr)
```


```{r}
DMLMatrix <- unique(merge(x = getData(differentialMethylationStatsFilteredCov5Destrand), y = getData(methylationInformationFilteredCov5Destrand), by = c("chr", "start", "end"))) #Merge DML list and all methylation information by DML chr, start, and end position. Use getData() to extract data from methylBase objects
DMLMatrix <- DMLMatrix[,-c(5:9)] #Remove duplicate columns
head(DMLMatrix) #Confirm merge
length(DMLMatrix$chr)
```
...
summary(DMLDataPCA) #Look at summary statistics. The first PC explains 18.1% of variation, the second PC explains 11.7% of variation
```
